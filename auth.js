    ...
    getUsers: function(req, res) {
        user.find({}, function (err, users) {
            ...
            users.forEach(function(entry){
                resToBeSent.push({'username': entry.username, 'name': entry.name, 'uId': entry.uId, 'emailId' : entry.emailID});
            });
    ...
    getInfo: function(req, res) {
        user.find({username: req.body.username}, function (err, userToGet) {
            userToGet.forEach(function (entry) {
                var info = {username: entry.username, name: entry.name, uId: entry.uId, emailId: entry.emailID};
            ...
            if (users.length === 0) {
                new user({
                    ...
                    if (err) {
                        ...
                        var newPath = nm_dir + "\\" + req.body.username;
                ...
                res.write("{'statusCode' : '404', 'statusMessage' : 'User Exists'}", function (err) {
                    res.end();
                });
            }
        });
